        ...
        <div class="card-body px-0 pb-2">
            <div class="table-responsive p-0">
                <table class="table align-items-center mb-0">
                    ...
                    <tbody>
                        @foreach($eventBookings as $index => $eventBooking)
                            <tr>
                                <!-- Sr No. -->
                                <td>
                                ...
                                <td>
                                    <div class="d-flex px-2 py-1">
                                        <h6 class="mb-0 text-sm">
                                            @if(isset($eventBooking->is_group) && $eventBooking->is_group)
                                                {{ $eventBooking->hall_name }}
                                                <br><small class="text-muted">({{ $eventBooking->group_count }} halls)</small>
                                            @else
                                                {{ $eventBooking->hall_name }}
                                            @endif
                                        </h6>
                                ...
                                <td class="align-middle text-center">
                                    <a href="{{ route('EventView.Booking', $eventBooking->id) }}" class="btn btn-secondary">
                                        View
                                    </a>
                                </td>
                                ...
                                <td class="align-middle text-center">
                                    @if ($eventBooking->eventServices->isEmpty() || $eventBooking->eventServices->pluck('status')->every(fn($status) => $status === 'pending' || is_null($status)))
                                        <div class="d-flex justify-content-end p-3">
                                            <a class="btn btn-outline-primary btn-lg px-4 py-2" href="{{ route('View.AddEventService', $eventBooking->id) }}">
                                                Add
                                            </a>
                                        </div>
                                    @endif
                                ...
                                <!-- Confirm Booking -->
                                {{-- @if ($eventBooking->eventServices->isEmpty() || !$eventBooking->eventServices->pluck('status')->contains('confirmed'))
                                <div class="d-flex justify-content-end p-3">
                                    <a class="btn btn-outline-primary btn-lg px-4 py-2" href="">
                                        Confirm Booking
                                    </a>
                                </div>
                                @endif --}}
                                 ...
                                 <td class="align-middle text-center">
                                     @php
                                         $status = $eventBooking->eventServices->pluck('status')->unique();
                                     @endphp
                                ...
                                <td class="align-middle text-center">
                                    ...
                                    <form action="{{ route('confirm.event', $eventBooking->id) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="btn btn-outline-success btn-lg px-4 py-2">Confirm Event Booking</button>
                                    </form>
                                    @endif
                                </td>
