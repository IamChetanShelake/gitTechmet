...
<body>
    <div class="container mt-5 d-flex justify-content-center">
        <div class="card shadow-lg p-5 w-75">
            <h2 class="text-center mb-4 text-primary">Add Key Feature</h2>
            <form action="{{ route('Add.Key') }}" method="POST" enctype="multipart/form-data">
                @csrf

                <div class="mb-3">
                    <label for="title" class="form-label fw-bold">Key Feature</label>
                    <input type="text" class="form-control border rounded-3 shadow-sm ps-3 @error('title') is-invalid @enderror"
                           id="title" name="title" placeholder="Enter Key Feature..." required>
                    @error('title')
                ...
                <div class="d-flex justify-content-between mt-4">
                    ...
                    <a href="/keys" class="btn btn-outline-secondary px-4">Back</a>
                </div>
            </form>
        </div>
    </div>
