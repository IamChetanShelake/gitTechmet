            ...
            <form action="{{ route('admin.events.update', $event->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control border rounded-3 shadow-sm ps-3 @error('title') is-invalid @enderror"
                           id="title" name="title" placeholder="Enter event title..." value="{{ old('title', $event->title) }}" required>
                    @error('title')
                ...
                <div class="mb-3">
                    ...
                    <textarea type="text" class="form-control border rounded-3 shadow-sm ps-3"
                              name="description" id="description" placeholder="Write a detailed description of the event..." rows="6">{{ old('description', $event->description) }}</textarea>
                    @error('description')
                    <span class="invalid-feedback d-block">
                        {{$message}}
                    </span>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="time" class="form-control border rounded-3 shadow-sm ps-3 @error('event_time') is-invalid @enderror"
                               id="event_time" name="event_time" value="{{ old('event_time', $event->event_time ? $event->event_time->format('H:i') : '') }}">
                        @error('event_time')
                        <span class="invalid-feedback d-block">
                            {{$message}}
                        </span>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control border rounded-3 shadow-sm ps-3 @error('location') is-invalid @enderror"
                        ...
                        @foreach($halls as $hall)
                            <option value="{{ $hall->name }}" {{ old('location', $event->location) == $hall->name ? 'selected' : '' }}>
                ...
                <div class="mb-3">
                    ...
                    @if($event->image)
                        <div class="mt-2">
                            <img src="{{ asset('Event_images/' . $event->image) }}" alt="Current Image" class="img-thumbnail" style="max-width: 200px;">
                            <p class="text-muted mt-1">Leave empty to keep current image</p>
                        </div>
                    @endif
                    @error('image')
                    <span class="invalid-feedback d-block">
